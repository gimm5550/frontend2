    ...
    useEffect(() => {
        const fetchTravelData = async () => {
            try {
                ...
                const publicData = response.data.filter(item => item.visibility === 'public');
                console.log("Public 데이터:", publicData);
                setTravelData(publicData); // 서버 응답 데이터를 상태에 저장
                setFilteredData(publicData); // 초기 상태로 필터링 데이터도 설정
            } catch (error) {
