...
export default function GoogleMapPage() {
    ...
    const fetchTouristConcentrationInfo = async (areaCd, sigunguCd, tAtsNm) => {
        ...
        try {
            ...
            //const response = await fetch(`${baseURL}?${params}`);
            console.log("11");
            const response = await fetch("http://apis.data.go.kr/B551011/TatsCnctrRateService/tatsCnctrRatedList?serviceKey=B6yoGtpONP7teTz9XRQHgw2WyPG3EipHt/hWAGbHZ5j9Jgl0rOt4LEAW+AnCxu8zmyLc0y00dCpBzXgesBG2Wg==&numOfRows=30&pageNo=1&MobileOS=ETC&MobileApp=AppTest&areaCd=51&signguCd=51130&tAtsNm=간현관광지");
            console.log("22");
            const data = await response.json();
            console.log("33");
            if (data.response && data.response.body) {
