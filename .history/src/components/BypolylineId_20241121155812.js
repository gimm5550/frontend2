...
export default function BypolylineId() {
    ...
    const handleLike = () => {
        setLikes((prevLikes) => {
            const updatedLikes = prevLikes + 1; // 이전 값 기반으로 증가
            console.log("updatedLikes 증가됨!!!", updatedLikes)
            api.likeupdate(polylineId, updatedLikes); // 증가된 값을 서버로 전송
            return updatedLikes; // 상태 업데이트
        });
    
        const fetchLikes = async (polylineId) => {
            try {
                const response = await api.getlike(polylineId); // 결과를 기다림
                console.log("like response:", response.data); // 이제 실제 값을 출력
            } catch (error) {
                console.error("Failed to fetch likes:", error);
            }
        };
    
        // 서버에서 최신 likes 값을 가져와 확인
        fetchLikes(polylineId);
    };
