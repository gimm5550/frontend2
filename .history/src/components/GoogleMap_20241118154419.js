                        ...
                        <Polyline
                            ...
                            onMouseUp={() => {
                                if (polylineRef.current) {
                                    ...
                                    //setPath(updatedPath); // 상태 업데이트 추가
                                    if (!isFirstDragHandled) {
                                        // 최초 실행 시에만 시작점과 끝점을 잇는 직선을 제거
                                        if (updatedPath.length > 2) {
                                            setPath(updatedPath.slice(0, -3)); // 마지막 점 제거
                                        } else {
                                            setPath(updatedPath); // 정상 경로 업데이트
                                        }
                                        setIsFirstDragHandled(true); // 이후로는 이 로직 실행 금지
                                    } else {
                                        // 이후 드래그 동작에서는 경로 업데이트만 수행
                                        setPath(updatedPath);
                                    }
                                }
