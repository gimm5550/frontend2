...
export default function GoogleMapPage() {
    ...
    const handleMarkerClick = (index) => {
        setSelectedPlace(markers[index]);
    };
    ...
    return (
        <div style={{ height: "100vh", width: "100%", display: "flex", flexDirection: "column" }}>
                ...
                >
                    ...
                    {markers.map((marker, index) => (
                        <Marker
                            ...
                            onRightClick={() => handleMarkerRightClick(index)} // 마커 우클릭 시 삭제
                        />
                    ))}
                    {places.map(place => (
                        <Marker
            ...
            <div style={{ width: "300px", backgroundColor: "#f9f9f9", padding: "10px", borderLeft: "1px solid #ddd" }}>
                <h3>선택한 위치 상세 정보</h3>
                {selectedPlace ? (
                    <div>
                        <p><strong>위도:</strong> {selectedPlace.position.lat}</p>
                        <p><strong>경도:</strong> {selectedPlace.position.lng}</p>
                        <textarea
                            value={selectedPlace.memo}
                            placeholder="메모를 입력하세요"
