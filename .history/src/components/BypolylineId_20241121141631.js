...
export default function BypolylineId() {
    ...
    const [photoPath, setPhotoPath] = useState(""); // 사진 경로 상태 추가
    useEffect(() => {
        if (!polylineId) {
            return;
        }
    ...
    // postContent와 photoPath 변경 시 로그 출력
    useEffect(() => {
        console.log("Updated PostContent:", postContent);
        console.log("Updated PhotoPath:", photoPath);
    }, [postContent, photoPath]);
    useEffect(() => {
    ...
    return (
        <div style={{ display: "flex", height: "100vh", width: "100%" }}>
            ...
            <div>본문내용: {postContent}</div>
            <div>사진: {photoPath}</div>
        </div>
    );
}
