...
export default function CommunityView() {
    ...
    const fetchCommunityData = async () => {
        try {
            ...
            console.log("community[0]:", community[0])
        } catch (err) {
        ...
        <div className="community-view"> {/* 여기에 클래스 추가 */}
            ...
            {community.length > 0 ? ( // 배열이 비어있지 않을 때만 출력
                <>
                    ...
                    <h3>댓글 목록</h3>
                    <ul>
                        {community[0].id.map(comment => (
                            <li key={community[0].id}>
                                <strong>{community[0].comment.nickname}:</strong> {community[0].comment.content}
                            </li>
                        ))}
                    </ul>
