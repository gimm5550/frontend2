...
export default function GoogleMapPage() {
        ...
        try {
            ...
            if (data.response && data.response.body) {
                ...
                if (items.length > 0) {
                    ...
                    // 디버깅 로그
                    console.log("Computed areacode: ", sigunguCodePrefix);
                    console.log("Computed sigungucode2: ", sigunguCode);
    ...
    const fetchTouristConcentrationInfo = async (areaCd, sigunguCd, tAtsNm) => {
        ...
        const baseURL = "http://apis.data.go.kr/B551011/TatsCnctrRateService";
        ...
        const params = new URLSearchParams({
            ...
            areaCd : areaCd, // 지역 코드
            sigunguCd : sigunguCd, // 시군구 코드
            tAtsNm : tAtsNm // 관광지명
        });
    
        try {
            const service_key = "B6yoGtpONP7teTz9XRQHgw2WyPG3EipHt%2FhWAGbHZ5j9Jgl0rOt4LEAW%2BAnCxu8zmyLc0y00dCpBzXgesBG2Wg%3D%3D";
const response = await fetch(
    `http://apis.data.go.kr/B551011/TatsCnctrRateService/tatsCnctrRatedList?serviceKey=${service_key}&numOfRows=30&pageNo=1&MobileOS=ETC&MobileApp=AppTest&areaCd=51&signguCd=51130&tAtsNm=간현관광지&_type=json`
);

        
            const textData = await response.text(); // 응답 데이터를 텍스트로 읽기
            ...
            const cnctrRates = Array.from(items).map(item => {
                const cnctrRate = item.getElementsByTagName("cnctrRate")[0].textContent;
                return cnctrRate;
            });
        
            console.log("cnctrRates: ", cnctrRates);
        } catch (error) {
                    ...
                    {selectedPlace && (
                        <InfoWindow
                            ...
                            onLoad={() => {
                                ...
                                fetchTouristInfo(lat, lng, selectedPlace.name);
                            }}
