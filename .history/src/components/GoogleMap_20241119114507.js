        ...
        try {
            ...
            if (data.response && data.response.body) {
                ...
                if (items.length > 0) {
                    ...
                    console.log("Region: ", region); // 지역 이름 출력
                    console.log("Sigungu Code: ", sigunguCode); // 시군구 코드 출력
                    const sigunguCodePrefix = sigunguCode.toString().slice(0, 2); // 시군구 코드의 앞 두 자리 추출
                    console.log("Sigungu Code Prefix: ", sigunguCodePrefix); // 앞 두 자리 출력
                    ...
                    {selectedPlace && (
                        <InfoWindow
                            ...
                            onLoad={() => {
                                ...
                                console.log("InfoWindow Position: ", { lat, lng });
                                fetchTouristInfo(lat, lng); // 위치 기반 관광 정보 검색 호출
                                fetchTouristConcentrationInfo(areacode, sigungucode2, "종로구"); // fetchTouristConcentrationInfo("11", "11110", "종로구");
                            }}
