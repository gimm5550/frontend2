...
export default function MainList() {
    ...
    const handleEnterMyTravel = () => {
        navigate('/MyTravel'); // 지정된 경로로 이동
    };

    const handleEnterTravel = () => {
        navigate('/Travel'); // 지정된 경로로 이동
    };

    return (
        <div>
            <button onClick={() => handleEnterGoogleMap()}>구글맵</button>
            <button onClick={() => handleEnterMyTravel()}>내 여행기록</button>
            <button onClick={() => handleEnterTravel()}>전체 여행기록</button>
            <h1>여행 기록 목록</h1>
                ...
                <tbody>
                    {travelRecords.map(record => (
                        <tr key={record.id}>
                            ...
                            <td>
                                <button onClick={() => handleEnterCommunity(record.id)}>입장</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
