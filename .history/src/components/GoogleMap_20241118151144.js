...
export default function GoogleMapPage() {
    ...
    const [isInitialCorrectionDone, setIsInitialCorrectionDone] = useState(false); // 최초 수정 여부
                        ...
                        <Polyline
                            ...
                            onMouseUp={() => {
                                if (polylineRef.current) {
                                    ...
                                    // 중간 점이 추가된 경우, 반대 방향의 점을 제거
                                    // 최초 한 번만 반대 방향의 점 제거 로직 수행
                                    if (!isInitialCorrectionDone && updatedPath.length > path.length) {
                                        setPath(updatedPath.slice(0, updatedPath.length - 1)); // 맨 마지막 점 제거
                                        setIsInitialCorrectionDone(true); // 수정 완료 상태로 설정
                                    } else {
                                        setPath(updatedPath); // 정상 경로 업데이트
                                    }
                                }
                            }}
                        />
