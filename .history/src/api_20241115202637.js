...
export default {
    ...
    getpassword(userId) {
        // userId가 객체라면, id 값을 추출
        const extractedUserId = typeof userId === 'object' && userId.id ? userId.id : userId;
    
        console.log("Sending userId as query parameter. userId:", extractedUserId);
        return instance.get('/api/edit/EditInfo', {
            params: { userId: extractedUserId } // 문자열로 전달
        });
